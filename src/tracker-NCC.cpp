...
void dialog() {
  ...
  for (;;) {
    if (token == "help") {
      ...
      cout << "  read_image  ..................... Read an image by using magick++" << endl;
      ...
      cout << "  create_thumb .................... Create a thumb image" << endl;
...
int save_grains(const char *name, int num, bool simpleVersion) {
  ...
  if (simpleVersion) {
    ...
    grain_file_out << "CorrelatedImages: " << fname << ' ';
  ...
  while (command_file) {
    ...
    if (token[0] == '!')
      ...
      if (token == "circ") {
        ...
        // Same pattern for each grain
        for (int i = 0; i < num_grains; i++)
          if (targetRadiusPattern < 0.0 || grain[i].radius_pix == targetRadiusPattern) make_ring_pattern(i, radius_IN, radius_OUT);
      } else if (token == "file") {
        char filename[256];
        command_file >> filename;
        ...
        // Same pattern for each grain
        for (int i = 0; i < num_grains; i++)
          if (targetRadiusPattern < 0.0 || grain[i].radius_pix == targetRadiusPattern) make_rect_pattern(i, half_width_pix, half_height_pix);
      }
    }
