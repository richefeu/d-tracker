// A c++ version of powell minimization in multidimensions.
...
template <class T> struct Powell : Linemethod<T> {
  ...
  vector<double> minimize(vector<double> &pp, vector<double> &xivec) {
  ...
  vector<double> minimize(vector<double> &pp, vector<vector<double>> &ximat) {
    const int ITMAX = 200;
