...
c-----calcul de la moyenne des pos et rot de tous les pts fixes 
      Open(30,file='checktranslationPTFIX.txt',status='unknown')
      Do num_photo = 1,NbrePhotos
         depx = 0.d0
         depy = 0.d0
         rot = 0.d0
         Do iptfix = 1,npt_fix
            depx = depx + pt_fix(1,iptfix,num_photo)
            depy = depy + pt_fix(2,iptfix,num_photo)
            rot = rot + pt_fix(3,iptfix,num_photo)
         EndDo
         transf(1,num_photo) = depx / dfloat(npt_fix)
         transf(2,num_photo) = depy / dfloat(npt_fix)
         transf(3,num_photo) = rot / dfloat(npt_fix) 
         Write(30,*) transf(1,num_photo),transf(2,num_photo),
     &               transf(3,num_photo)
      EndDo
      Close(30)
...
c-----Calcul des deformations a partir des 4 coins (hyp: grandes def)
      ...
      Call ComputeDefHPP(NbrePhotos,r_coins,Fxx,Fxy,Fyx,Fyy,Exx,Exy,
     &                Eyy,Evol,Evide)

c-----Creation des fichiers CONF
      FichConf1 = 'CONF%04d'
      Write(6,*) 'Writing CONF%04d files'
      Do num_photo = 1,NbrePhotos
...
c        -- correction mouvement camera et mise a l'echelle
         Call TransfGrains(num_photo,transf,ngrains,r_grains,
     &        fact_echelle)
...
c      If (idisto.EQ.0) Then ! real undisto
c         Write(6,*) ' --> fake_undistor = ', idisto    
c         Write(6,*) 'real undedistorsion'
c         Write(6,*) 'Not yet ready -- sorry'
c         Write(6,*) ' - - - - - - - - - - - - - - -'
c	 Call Exit(1)
c      EndIf
c     --> idisto = 1  ! fake undistorsion 
