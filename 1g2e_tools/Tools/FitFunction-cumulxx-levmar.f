...
c=============================================================
c     Calcul des coefficients d'une qgaussienne sur des donnees
c     de fluctuation. 
c     
c     Fit effectue sur la densite de probabilite cumulee
c
c     accepte un max de 5 millions de valeurs
c=============================================================

      Program FitFunction_levmar

c      Implicit Real*8 (a-h,o-z)
      Parameter(Lmax=5000000,lwam = 10*Lmax)
      Common/tab/ Val(3,Lmax),nval
      Character*50 Fichier
      Real*4 input(Lmax),fit(Lmax)
      Integer j,m,n,info,lwa
      Integer iwa(4)
      Real*8 Tol,Fnorm
      Real*8 x(4),fvec(Lmax),wa(lwam)
      Real*8 Enorm,dpmpar
      External fcn
      Real*8 tab1(Lmax),tab2(Lmax),dx,qq,xxx,qgauss
...
1     Read(1,*,end=2) z1
          k = k + 1
          If (k.GT.Lmax*0.5) Write(6,*) 'Lmax trop faible'
          val(1,k) = z1
          GoTo 1
2     Rewind(1)
      nval = k
...
c-----echantillonnage
      ...
      Write(6,*) 'Traitement sur donnees brutes ? (0=oui)'
      Read(5,*) irep_brute
      If (irep_brute.NE.0) Then
         Write(6,*) 'ECRETAGE :'
         Write(6,*) "xmin,xmax = ",xmin,xmax
         Write(6,*) "xrange ? (x1min,x1max)" 
         Read(5,*) x1min,x1max
         Write(6,*) x1min,x1max
         Write(6,*) 'Symmetrisation ? (0=oui)'
         Read(5,*) irep_sym
         k = 0
10       Read(1,*,end=20) z1,z2,z3
             zz = z1
             If ((zz.GE.x1min).AND.(zz.LE.x1max)) Then
                k = k + 1
                val(1,k) = zz 
                If (irep_sym.EQ.0) Then
                   k = k + 1        !symetrisation
                   val(1,k) = -zz   !symetrisation
                EndIf
             EndIf
         GoTo 10
20       Close(1)
      Else
         Write(6,*) '-->traitement sur donnees brutes !'
         k = 0
11       Read(1,*,end=21) z1,z2,z3
             zz = z1
             k = k + 1
             val(1,k) = zz 
         GoTo 11
21       Close(1)
      ...
      nval = k
      Write(6,*) 'Nbre de points :',nval
      If (irep_sym.EQ.0) Write(6,*) 'les donnees ont ete symetrisee'

      Do i = 1,nval
         tab1(i) = val(1,i)
         tab2(i) = dfloat(i)/dfloat(nval)
      EndDo      
      Call Sort(nval,tab1)
      Do i = 1,nval
         val(1,i) = tab1(i)
         val(2,i) = tab2(i)
         Write(11,*) val(1,i),val(2,i)
      EndDo
         
c-----Valeurs initiales pour le levmar
      n = 4
      m = nval
      lwa = n*m + 5*n + m + 1
      x(1) = 10.d0
      ...
      Write(6,*) 'recherches de coeff...'

      tol = 1.d-8
      Call lmdif1(fcn,m,n,x,fvec,tol,info,iwa,wa,lwa)

      fnorm = enorm(m,fvec)
      Write(6,*) 'fnorm,info = ',fnorm,info!,(x(j),j=1,n)
...
c-----Verification - coeff de correlation de pearson
      ...
      i = 1
      xxx = val(1,i)
      dx = (val(1,i+1) - val(1,i))*0.5d0
      qgauss = xa*(1.d0-(1.d0-xq)*xb*(xxx-x0)*(xxx-x0))**
     &         (1.d0/(1.d0-xq))
      qq = qq + qgauss * dx
      fit(i) = qq
      input(i) = val(2,i)
      Do i = 2,nval - 1
         ...
         qgauss = xa*(1.d0-(1.d0-xq)*xb*(xxx-x0)*(xxx-x0))**
     &            (1.d0/(1.d0-xq))
         qq = qq + qgauss * (val(1,i+1)-val(1,i-1))*0.5d0
         fit(i) = qq
         input(i) = val(2,i)
      EndDo
      i = nval
      xxx = val(1,i)
      dx = (val(1,i) - val(1,i-1))*0.5d0
      qgauss = xa*(1.d0-(1.d0-xq)*xb*(xxx-x0)*(xxx-x0))**
     &         (1.d0/(1.d0-xq))
      qq = qq + qgauss * dx
      fit(i) = qq
      ...
      Write(6,*) "r = ",r
      Write(6,*) "r2 = ",r*r
c      Call kstwo(input,nval,fit,nval,d,prob)
c      Write(6,*) "prob KSTWO = ", prob
...
c=============================================================
      Real Function fcn(m,n,x,fvec,iflag)
      Parameter(Lmax=5000000)
      Common/tab/ Val(3,Lmax),nval
      Real*8 x(n),fvec(m),xxx,x0,xa,xb,xq,yyy,xp
      Real*8 qgauss,qq,qcumul(Lmax),dx
     ...
     &         (1.d0/(1.d0-xq))
      ...
      sum = 0.
      Do i = 1,nval
c         fvec(i) = ((qcumul(i)) - (val(2,i))) 
          fvec(i) = abs(log10(qcumul(i)) - log10(val(2,i))) 
          sum = sum + fvec(i)
      EndDo
      Write(6,*) 'err cumul,q =',sum,xq
...
c     **********
     ...
     *     /1.0d0,1.0d-1,5.0d-1,2.5d-1,7.5d-1,1.0d-6,0.0d0/
c
c     epsmch is the machine precision.
c
      epsmch = dpmpar(1)
...
c      z=0.5*log(((1.+r)+TINY)/((1.-r)+TINY))
c      df=n-2
c      t=r*sqrt(df/(((1.-r)+TINY)*((1.+r)+TINY)))
