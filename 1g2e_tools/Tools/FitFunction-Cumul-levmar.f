...
c-----Valeurs initiales pour le levmar
      ...
      fnorm = enorm(m,fvec)
      Write(6,*) fnorm,info,(x(j),j=1,n)


      Write(6,*) "a = ", x(1)
      Write(6,*) "b = ", x(2)
      Write(6,*) "q = ", x(3)
      Write(6,*) "f(x) = a*(1.-(1-q)*b*x*x)**(1./(1-q))" 
...
c=============================================================
c      Real Function fcn(m,n,x,fvec,iflag)
c      Parameter(Lmax=5000000)
c      Common/tab/ Val(3,Lmax),nval
c      Real*8 x(n),fvec(m),xxx,xa,xb,xq,yyy
c
c      xa = x(1)
c      xb = x(2)
c      xq = x(3)
...
c         yyy = Val(2,i)
c         fvec(i) = yyy - 
c     &             xa*(1.d0-(1.d0-xq)*xb*xxx*xxx)**(1.d0/(1.d0-xq))
c         fvec(i) = fvec(i) / 1.!Val(3,i)
c      EndDo
...
c      End
      ...
      Common/tab/ Val(3,Lmax),nval
      Real*8 x(n),fvec(m),xxx,xa,xb,xq,yyy
      Real*4 a,b,c,z
      ...
      Do i = 1,nval
         xxx = Val(1,i)
         z = -xb*(xq-1.)*xxx*xxx
         yyy = dfloat(i)/dfloat(nval)
         fvec(i) = yyy - a*xxx*hypgeo(a,b,c,z)
      EndDo
...
C  (C) Copr. 1986-92 Numerical Recipes Software v%1jw#<0(9p#3.
     ...
     *yp(NMAX,KMAXX),yscal(NMAX)
      COMMON /path/ kmax
      COMMON /path2/ kount,dxsav,xp,yp
      x=x1
      h=sign(h1,x2-x1)
      nok=0
      nbad=0
